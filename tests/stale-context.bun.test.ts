...
async function importFreshCodexUsageExtension(label: string): Promise<{
	...
	await cp(new URL("../extensions", import.meta.url), path.join(moduleDir, "extensions"), { recursive: true });
	await cp(new URL("../src", import.meta.url), path.join(moduleDir, "src"), { recursive: true });

	const modulePath = path.join(moduleDir, "extensions", "codex-usage-status.ts");
